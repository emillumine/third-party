var main_content = $('#main-content'),
    ...
    adding_pdts_tpl = $('#adding-products').clone().removeAttr('id'),
    active_phase = 'main',
                          ...
                          function(err, result) {
                            if (!err) {
                                if (typeof result.res !== "undefined" && result.res == true) {
                                    shelfs_table.row(clicked.parents('tr')).remove().draw()
...
var addProductToList = async function(barcode) {
    ...
    if (odoo_product === null) {
      ...
      var pdt_line = $('<tr>').attr('data-bc',odoo_product.barcode).addClass('obc')
      $('<td>').text(barcode).appendTo(pdt_line)
      $('<td>').text(odoo_product.barcode).appendTo(pdt_line)
      $('<td>').text(odoo_product.data[0]).appendTo(pdt_line)
      $('<td>').html(delete_icon).appendTo(pdt_line)
      adding_pdts_tpl.find('#added_products tbody').append(pdt_line)
      main_content.find('button.add-products').css('display','block').html(add_pdts_btn_text)
    }
}
...
var addProducts = async function() {
    ...
    adding_pdts_tpl.find('.shelf').text(data.name + ' (num = ' + data.sort_order+')').attr('data-shelfid',data.id)
...
var recordProductsAddedShelf = function() {
    ...
    if (to_add.length > 0) {
        ...
        if (is_time_to('add_pdts_to_shelf', 5000)) { // prevent double click or browser hic up bug
                ...
                post_form('/shelfs/admin/add_products',
                ...
                .done(function(rData){
                    if (typeof rData.res.data != "undefined" && rData.res.data.length > 0) {
                        ...
                        $.each(rData.res.data, function(i,pdt){
                            var tr = $('<tr>')
                            var td1 = $('<td>').css('width','100px').text(pdt.barcode)
                            var td2 = $('<td>').css({'text-align':'left','padding-left':'10px'}).text(pdt.name)
                            tr.append(td1)
                            tr.append(td2)
                            table.append(tr)
                        })
  ...
  if (coop_is_connected()) {
      ...
      try {
          if ( admin_ids.find(id => id == getCookie("uid"))) {
              $(document).on('click','.add-search .fa-eye', function(){addSearchDisplay('show')})
              $(document).on('click','.add-search .fa-eye-slash', function(){addSearchDisplay('hide')})
