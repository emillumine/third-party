...
function display_msg_box(msg, type) {
    ...
    var mcont = $('<div/>').addClass('content '+ mtype).html(msg);
...
String.prototype.toFormatedFirstName = function(){
  ...
  if (typeof force_fn_hyphen != "undefined" && force_fn_hyphen == true) {
      formatted = this.trim().replace(/-+/g,' ').toCapital().replace(/ +/g,'-')
  }
  return formatted
}
...
var btn_nok = $('<button/>').addClass('btn--danger').attr('id', 'modal_closebtn_bottom').text('Fermer');
...
function openModal() {
  ...
  if (arguments[0]){
    ...
    var oc = $('<div/>').addClass('mconfirm').html(arguments[0])
...
function displayMsg(content) {
   ...
   modal.find('.overlay-content').html($('<div/>').addClass('mconfirm').html(content))
...
function coop_authenticate(callback){
    post_form('/members/get_credentials',
              {'login': $('input[name="login"]').val(),
               'password': $('input[name="password"]').val()
                 ...
                 if (typeof (result.authtoken) != "undefined") {
                    ...
                    var msg = $('<div>').addClass('error_msg').text('Identifiants inconnus ou erreur dans la saisie');
...
if (getCookie('deconnect_option') && $('#deconnect').length == 0) {
    ...
    $('body').prepend($('<button>').attr('id','deconnect').attr('type','button').text('Déconnexion'));
