  ...
  try {
    ...
    if (products_table == null) {
      ...
      $('#products_table tbody').on('click', 'button.stock_edit_button', function () {
        ...
        // Data validation
        qty = $(this).parents('tr').find('input').val()
      ...
      // Add row to table
      var rowNode = products_table.row.add(product).draw(false).node();
  ...
  $.ajax({
    ...
    error: function(data) {
      if (data.status == 404) {
        ...
        $.notify("Aucun produit trouvé avec ce code barre !",
          {
            globalPosition:"top left",
            className: "error"
          }
