  ...
  $(document).ready(function() {
    ...
    var del_member_from_list = function() {
      ...
      if (is_time_to('del', 1000)) {
         openModal("L'opération est irréversible, êtes-vous sûr de vouloir le supprimer ?",
                   function() {
                     try {
                        //Call django method (instead of pouchdb call)
                        post_form('/members/remove_member_from_mess_list',
         ...
         if (mdata != null && is_time_to('generate_envelops')) {
            ...
            openModal(form_tempate.html(),
                      function() {
                        ...
                        post_form('/members/create_envelops',new FormData(form.get(0)),
                                  function(err,rData){
            ...
            $('.mconfirm').find('[name="shares_euros"]').val(mdata.doc.data.amount_subscription)
         }
    }
    ...
    var ask_for_add_to_list = function() {
       ...
       openModal(msg, 
                 function() {add_res_to_list(clicked.attr('data-jss'))},
                'oui')
    }
