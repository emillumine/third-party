...
function put_current_coop_in_buffer_db(callback) {
   ...
   if (typeof current_coop._old_id != "undefined"){
      dbc.remove(current_coop._old_id,current_coop._rev,function(err,response){
          if (err){console.log(err);can_continue = false;}
      });
...
function save_current_coop(callback) {
  ...
  if (current_coop != null && _id.length > 0) {
    ...
    let birthdate = form.find('[name="birthdate"]').val().trim()
    ...
    current_coop.firstname = form.find('[name="firstname"]').val().toFormatedFirstName();
    current_coop.lastname = form.find('[name="lastname"]').val().toFormatedLastName();
...
function coop_form_delete() {
  try {
    ...
    post_form('/members/remove_data_from_couchdb',
...
function add_coop_to_box(box,coop) {
  ...
  info = $('<span/>').text(info)
  var cbox = $('<div/>').addClass('coop').attr('data-id',coop._id).append(info);
  // Only connected user can do these actions
  if (["waiting_validation_employee", "waiting_validation_member", "done"].indexOf(coop.validation_state) > -1
      && !coop_is_connected()) {
