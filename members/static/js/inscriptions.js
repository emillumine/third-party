    ...
    dbc.put(coop, function callback(err, result) {
        if (!err) {
           ...
           if (typeof coop.shift_template != "undefined") {
              openModal ('Voulez-vous modifier le créneau choisi ?', swipe_to_shift_choice, 'oui',
...
function store_new_coop(event){
    ...
    let email = $('input[name="email"]').val().trim(),
        fname = $('input[name="firstname"]').val().toFormatedFirstName(),
        lname = $('input[name="lastname"]').val().toFormatedLastName()
    ...
    if (errors.length == 0) {
       _really_save_new_coop(email, fname, lname,
...
function modify_c_shift(){
    var clicked = $(this)
    var coop_id = clicked.closest('tr').find('td.coop div').data('id')
...
function show_coop_list(){
 if (current_coop == null || typeof (current_coop.shift_template) != "undefined") {
    ...
    $.each(self_records, function(i,e){
    var tr = $('<tr/>');
    var coop = $('<div/>').attr('data-id',e._id).html(e.firstname + ' ' + e.lastname + ' - '+e._id);
    var s_t = ''
    if (e.shift_template) {
    ...
    tr.append($('<td/>').addClass('coop').append(coop));
    tr.append($('<td/>').addClass('c_shift').append(shift));
    table.append(tr)
    });
...
function generate_email(){
   var fname = $('input[name="firstname"]').val().toFormatedFirstName();
   var lname = $('input[name="lastname"]').val().toFormatedFirstName();
   if (fname.length > 0 && lname.length > 0) {
    ...
    //get out accents
    email = email.normalize('NFD').replace(/[\u0300-\u036f]/g, "").replace(/ +/g,'-')
