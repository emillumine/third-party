var main_content = $('#main-content'),
    ...
    loading_img = $('#rotating_loader').clone().removeAttr('id').addClass('rotating_loader'),
  ...
  var sorted_products = divs_to_sort.sort(function (a, b) {
    switch (sort_type) {
      ...
      case 'name_desc':
        return ($(a).find(".name").text() < $(b).find(".name").text()) ? 1 : -1
      case 'price_unit_asc':
        return parseFloat($(a).find("span.price").text()) > parseFloat($(b).find("span.price").text()) ? 1 : -1
      ...
      case 'price_uom_asc':
        ...
        var a_price_uom = $(a).find("span.uom_price").text() == 'Unité(s)' ? parseFloat($(a).find("span.uom_price").text()) : parseFloat($(a).find("span.price").text())
        var b_price_uom = $(b).find("span.uom_price").text() == 'Unité(s)' ? parseFloat($(b).find("span.uom_price").text()) : parseFloat($(b).find("span.price").text())
...
var getMaxTimeBeforeValidation = function() {
    var max_time = 1
    try {
        const selected_day = $('.overlay-content [name="bday"]').val().replace(/-/g, "/") // replace for Safari
...
var getStartTimeAndEndTimeForShortDay = function(short_day) {
    ...
    try {
        for (i in opening[short_day]) {
                    var [sh,sm] = opening[short_day][i]['start'].split(':')
                    var [eh,em] = opening[short_day][i]['end'].split(':')
                    var s = new Date()
                    var e = new Date()
                    s.setHours(sh)
                    s.setMinutes(sm)
...
var generateHourOptions = function() {
    ...
    $.each(options, function(i,e) {
        hours_sel.append($('<option>').val(e.val).text(e.text))
    })
}
...
var fillBDayOptions = function(select) {
    ...
    select.append($('<option>').attr("value","").text("---> Jour"))
    date_options = {weekday:'long', day: 'numeric', month: 'long'}
    var opening_days = ['mar.', 'mer.', 'jeu.', 'ven.', 'sam.']  // default for previous code compat.
    if (typeof opening != "undefined") {
        ...
        if (!closing_dates.includes(d.toISOString().slice(0,10))) {
          ...
          if (opening_days.indexOf(short_day) > -1) {
              ...
              if (is_possible_day(date, short_day))
                select.append($('<option>').attr("value",date).text(date_text))
          }
        }
    }
}
...
var putAlimCategData = function() {
    ['epicerie', 'liquide', 'produits_frais', 'surgeles'].forEach(function(k){
...
var putNonAlimCategData = function() {
    ['bazar', 'droguerie', 'parfumerie'].forEach(function(k){
        var div = categ_menu_template.clone().attr('data-id',categories[k].id)
        div.find('.dropbtn').text(categories[k].label)
    ...
    try {
        if (order.state == "init") {
            ...
            if (qty > 0) {
                ...
                var u = p_div.find('.unit').text().substring(0,1)
                if (u == "U") {
                    qty = parseInt(qty, 10)
                } else {
...
var isChoosenSlotValid = function(slot) {
    ...
    forbidden_slots.forEach(function(e){
        if(slot == e) { answer = false; cause = 'full';}
    })
    ...
    if (slot_date - min_date < 0)  { answer = false; cause = 'delay';}
...
var initCart = function () {
    ...
    if (allowed_slot.res == false || bd.length == 0 || bh.length == 0) {
        ...
        if (is_time_to('init_cart', 15000)) { // prevent double click or browser hic up bug
            ...
            post_form('/shop/cart_init', {order: JSON.stringify(order)},
                function(err,result){
                   if (!err) {
                     main_content.show()
...
var validCart = function() {
    ...
    post_form('/shop/cart', {order: JSON.stringify(order)},
                function(err,result){
                //console.log(result)
                   if (!err) {
                     try {
...
var appendProductsToGrid = function (grid, pdts, sort = true) {
    ...
    if (product_divs.length > 0) {
      if (sort) {
        ...
        var sort_type = grid.parent().find('select.products_sort option:selected').val()
      ...
      grid.parent().find('div.products_sort_container').show()
    }
}
...
var appendChildrenCatToMenu = function (catdiv, children) {
    ...
    $.each(children, function(i,e) {
        if (excluded_cat.indexOf(e.id) < 0) {
            ...
            var span = $('<span>').attr('data-id',e.id).text(e.name)
            li.append(span)
            ul.append(li)
        }
...
var getCategProducts = function() {
    ...
    var msg = $('#loading_categ_products_msg').clone().removeAttr('id')
    grid.empty()
                ...
                if (typeof rData.res.error != "undefined") {
                    ...
                    if (rData.res.data.orders.length > 0) {
                        ...
                        $.each(rData.res.data.orders,function(i,o) {
                            ...
                            var tr = $('<tr>').attr('data-id', o._id).attr('data-rev', o._rev)
                            if (o.state == "validating" || o.state == "init") {
                                actions_content = delete_icon
                                tr.prop('draggable', true)
                            }
                            var td1 = $('<td>').addClass('date create').text(date)
                            var td2 = $('<td>').addClass('date bdate').html(bdate_content)
                            var td3 = $('<td>').addClass('nb').text(o.products.length)
                            var td4 = $('<td>').addClass('amount').text(parseFloat(o.total).toFixed(2))
                            //var td5 = $('<td>').addClass('actions').html(eye + ' ' + delete_icon)
                            var td5 = $('<td>').addClass('actions').html(actions_content)
                            tr.append(td1)
                            tr.append(td2)
                            tr.append(td3)
                            tr.append(td4)
...
var validBDayChange = function(cart_id) {
    ...
    if (allowed_slot.res == false || bd.length == 0 || bh.length == 0) {
        ...
        if (is_time_to('change_date', 15000)) { // prevent double click or browser hic up bug
            ...
            setTimeout(function() {displayMsg(waiting_msg)}, 200) // delay needed because of closeModal called by confirm click
            post_form('/shop/cart/' + cart_id + '/change_date',
                      {new_date: bd + " " + bh},
                      function(err,result){
...
var changeBestDate = function() {
    ...
    updateUnavailableSlots(
        function(){
            openModal(msg.html(), function(){validBDayChange(id)},  'Enregistrer')
        }
    )
...
var sendFusionCartProposition = function(main_id, addid, date) {
    openModal('Enregistrer la fusion des 2 commandes<br/>Récupération le '+ french_date_and_time(date),
              function() {
                var waiting_msg = $('<p>').append(loading_img)
...
$('#deconnect').click(function(){
      $.ajax("/website/deconnect").done(function(){window.location.reload()})
})
