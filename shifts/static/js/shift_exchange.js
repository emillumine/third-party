...
function changeShift(idOldRegister, idNewShift){
  if (is_time_to('change_shift')) {
    ...
    if (idOldRegister == "") {
        ...
        register = listeShiftPartner.find(function(shift){return shift.id === Number(idOldRegister)});
...
$(document).ready(function() {
  ...
  $(document).on("click",".fa-info-circle",function(){
    var clicked = $(this);
    clicked.parent().find('>.infocircle').toggle();
  });
      ...
      if (event.changed){
        ...
        if (dataPartner.is_leave == "True" && (dateShiftNew < dateFinConge && dateShiftNew > dateDebutConge)){
          ...
          if (idShiftOld != "" && can_make_exchange) {
              var resOldShift = listeShiftPartner.find(function(shift){return shift.id === idShiftOld});
            ...
            if (!can_add_shift) {
              ...
              if (dataPartner.in_ftop_team == "True" || dataPartner.in_ftop_team == "False" && dateShiftNew > limitDate) {
                ...
                if (new Date() < date_end_alert) {
                  openModal("Pour pouvoir sélectionner un service à partir du <strong>" + limitDate.toLocaleDateString("fr-fr", optDate) + "</strong> je dois demander une extension.<br/>"
                  ...
                  openModal("Je dois sélectionner un service <strong>avant le " + limitDate.toLocaleDateString("fr-fr", optDate) + "</strong>",
                        closeModal, "J'ai compris"
                    );
                }
              } else {  // ABCD up-to-date
                openModal("Je dois sélectionner un service à échanger.",
                      closeModal, "J'ai compris"
                  );
              }
            // Can add shift
